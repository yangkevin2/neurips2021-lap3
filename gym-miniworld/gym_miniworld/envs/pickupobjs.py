    ...
    def _gen_world(self):
        ...
        while not acceptable:
            ...
            for obj in range(self.num_objs):
                ...
                box = self.place_entity(Box(color=color, size=0.1))
                ...
                # if obj_type == Ball:
                #     self.place_entity(Ball(color=color, size=0.5))
                # if obj_type == Key:
            ...
            acceptable = boxes_successful and self.box_dist > 3 and self.box_dist < 4
            if not acceptable:
    ...
    def step(self, action, print_eval=False):
        ...
        if done:
            ...
            for box, _ in self.boxes:
                dists.append(np.linalg.norm(self.agent.pos - box.pos)) # distances to boxes
            reward -= min(dists)
            for i, (box, _) in enumerate(self.boxes):
                # if self.near(box):
                if np.linalg.norm(self.agent.pos - box.pos) < 1:
                    # reward += 0.1*self._reward() + i # higher reward for going to a farther box
                    # reward += 0.01*self._reward() + i # higher reward for going to a farther box
                    reward += i
                    done = True
