...
class MiniGridEnv(gym.Env):
    ...
    def _reward(self, not_reaching=False):
        """
        ...
        if not not_reaching:
            ...
            for _i in range(self.grid.width):
                for _j in range(self.grid.height):
                    pos = [_i, _j]
                    if self.grid.get(*pos) and (self.grid.get(*pos).type == 'ball' or self.grid.get(*pos).type == 'goal'):
                        break
                if self.grid.get(*pos) and (self.grid.get(*pos).type == 'ball' or self.grid.get(*pos).type == 'goal'):
                    break
            assert (self.grid.get(*pos).type == 'ball' or self.grid.get(*pos).type == 'goal')
            return -((self.agent_pos[0] - _i)**2 + (self.agent_pos[1] - _j)**2)
    ...
    def step(self, action):
        ...
        if self.step_count >= self.max_steps:
            ...
            reward = self._reward(not_reaching=True)
